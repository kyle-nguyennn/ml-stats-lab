...
@modified_by: Zhaowei Meng (zmeng65) (2025-05-19)
Fixed some issues to run on Python 3.10
1. To accommodate the fact that meat.csv has neither a header nor an index column,
    replaced the following code:
        meat = pd.read_csv("meat.csv", index_col=0).values
    with:
        meat = pd.read_csv("meat.csv", header=None).values
2. Fixed version issues of Python:
    A) replaced the following code:
            ax = fig.gca(projection='3d')
        with:
            ax = fig.add_subplot(111, projection='3d')
        because the gca() method in Matplotlib no longer accepts the projection argument since version 3.4.
    B) set rcond=-1 to avoid the FutureWarning: `rcond` parameter will change to the default of machine precision times max(M, N) in the future.

3. plot B-Spline Basis and B-Matrix in one figure but two subplots
4. Refactored the code into the following separate functions for better readability and modular learning.
    # %% SPLINES
    chart_on_Slide_32()
...
def order_m_splines(m=4):
    ...
    # Polynomial Terms
    for mm in range(m):
        H.append(X.reshape(N, -1)**mm)

    # Truncated Power Functions
    for kk in k:
        H.append(np.maximum((X-kk)**(m-1), 0).reshape(N, -1))

    # for hh, hx in enumerate(H):
    #     print(f"f{hh+1}x = ")
    #     print(hx)
    ...
    # "Correct" way
    B = np.linalg.lstsq(H, Y, rcond=-1)[0]
    ...
    for kk in k:
        plt.axvline(x=kk, color='g')
    plt.ylim((-1.5, 1.5))
    plt.legend()
    plt.grid()
    plt.title(titles[m-1])
    plt.show()

# %% SPLINES
def chart_on_Slide_32():
    np.random.seed(0)
    # * CURVE FITTING
    # Data Generation
    X = np.linspace(0, 1, 1001)
...
def b_spline_basis():
    ...
    for degree in range(4):  # 0th order to cubic
        ...
        ax1.plot(Bs[-1], )
        ax1.set_ylim((0, 1.))
        ax1.set_title(f'B-Spline Basis, degree = {degree}')

        # Example code plots the coefficients of the cubic spline basis
        pos = ax2.matshow(Bs[-1], aspect=(nKnots+degree)/100, cmap='jet')  # type: ignore
        ax2.set_title('B-Matrix')
        ax2.set_xlabel('Spline index')  # index of spline
        ax2.set_ylabel('x')  # x-coordinate
...
def curve_fitting():
    ...
    B2 = BSplineBasis(x, np.linspace(0, 1, 10), 2)[:,:-2]
    B3 = BSplineBasis(x, np.linspace(0, 1, 10), 3)[:,:-2]

    for b_type, B in [('Quadratic ',B2),('Cubic', B3)]:
        # Least square estimation
        BBBB = B@np.linalg.inv(B.T@B)@B.T
        yhat = B@np.linalg.pinv(B)@ynoise  # More numerically stable
        # yhat2 = np.linalg.lstsq(B.T, B.T@ynoise, rcond=-1)[0]  # Probably the recommended way
        ...
        plt.legend()
        plt.title(f'Smoother Example ({b_type})')
        plt.show()


# * FAT CONTENT PREDICTION
def fat_content_prediction():
    # Baseline linear model
    nTest = 20

    # meat = pd.read_csv("meat.csv", index_col=0).values
    # Reads the file meat.csv without a header and without index column
    meat = pd.read_csv("meat.csv", header=None).values
...
def over_fitting():
    ...
    # Least Square Estimation:
    yhat = B@np.linalg.lstsq(B, y, rcond=-1)[0]
...
# %% FUNCTIONAL PRINCIPAL COMPONENTS
def functional_principal_components():
    ...
    X_stacked = X.ravel()
    beta = np.linalg.lstsq(B_stacked, X_stacked, rcond=-1)[0]
